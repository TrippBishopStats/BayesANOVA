...
model{
    for(i in 1:n){
        ...
        mu[i] <- beta1*I1[i] + beta2*I2[i]
       + beta3*I3[i] + beta4*I4[i] + beta5*I5[i]
    }#end for i
    ...
    ## derived parameters
    dOurVsBest <- beta1 - beta2 ## our - top
    dOurVsOthers  <- beta1 - (beta1 +beta2 + beta3 + beta4)/4 ## our - all others
}
"

## nice way to create the indicator variables
indicatorVars <- model.matrix(~ -1+type,data=dogFood)
...
params <- c('beta1','beta2','beta3','beta4','beta5',
